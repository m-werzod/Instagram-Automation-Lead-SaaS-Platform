  ...
  return (
    <div className={cn("mb-5 flex flex-wrap items-start justify-between gap-3", className)}>
      <div className="flex min-w-0 gap-3">
        {accent && <span className="mt-1 h-8 w-1 shrink-0 rounded-full" style={{ background: accent }} aria-hidden />}
        <div className="min-w-0">
          <h1 className="text-lg font-semibold leading-tight">{title}</h1>
          {description && <p className="mt-1 max-w-2xl text-xs leading-5 text-[--color-fg-muted]">{description}</p>}
        </div>
      </div>
...
export function EmptyState({
  ...
  return (
    <div className="rounded-lg border border-dashed border-[--color-border-strong] bg-[--color-panel] px-6 py-12 text-center">
